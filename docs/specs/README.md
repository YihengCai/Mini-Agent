...
最近完成的规格还包括 [后台 shell 状态与资源所有权](04-background-shell-ownership.md)、[模型可见工具输出预算](05-tool-output-budget.md)、[MCP 超时与连接的运行时所有权](06-mcp-runtime-ownership.md)、[严格配置解析与默认值单一真源](07-strict-config-loading.md)、[Note 存储的失败关闭边界](08-note-storage-integrity.md)、[配置伴随文件的来源边界](09-config-companion-provenance.md)、[MCP transport 的显式校验边界](10-mcp-transport-validation.md)、[Turn 日志的排他分配](13-exclusive-turn-logs.md)、[Skill 发现的注册表快照](14-transactional-skill-discovery.md)、[core 的模型失败语义](16-core-model-error-semantics.md)、[后台 shell 输出的完成边界](17-background-shell-output-completion.md)、[运行时工作区的单一所有权](18-runtime-workspace-ownership.md)、[工具返回值的接纳所有权](19-tool-result-ownership.md) 和 [前台 shell 中断时的直接子进程回收](20-foreground-shell-interruption-cleanup.md)。项目级 retry 已由 [ADR-0027](../decisions/0027-no-project-retry-before-error-classification.md) 删除，其旧规格不再作为现有能力入口。新的当前工作与未来问题只保留在 [`BUILD_LIST.md`](../BUILD_LIST.md)；进入相应实现时，再根据当时的代码、回归测试和端点探测结果写短规格。
