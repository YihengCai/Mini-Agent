...
最近完成的规格还包括 [后台 shell 状态与资源所有权](04-background-shell-ownership.md)、[模型可见工具输出预算](05-tool-output-budget.md)、[MCP 超时与连接的运行时所有权](06-mcp-runtime-ownership.md)、[严格配置解析与默认值单一真源](07-strict-config-loading.md)、[Note 存储的失败关闭边界](08-note-storage-integrity.md)、[配置伴随文件的来源边界](09-config-companion-provenance.md) 和 [MCP transport 的显式校验边界](10-mcp-transport-validation.md)。新的当前工作与未来问题只保留在 [`BUILD_LIST.md`](../BUILD_LIST.md)；进入相应实现时，再根据当时的代码、回归测试和端点探测结果写短规格。
