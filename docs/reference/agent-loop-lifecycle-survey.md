...
- 用途：记录外部实现怎样划分会话、一次执行和任务判定。本文提供实现前证据；实际选择与验证见 [ADR-0004](../decisions/0004-session-turn-step-lifecycle.md)。

## 本项目实现前的问题

改造前的 `Agent` 同时持有长期消息、工具、压缩状态、取消句柄和一次执行的步数控制。它不是每次新建的一次性对象，但公开边界仍是一次性的 `await run() -> str`，因此形成了“长期状态对象 + 一次性调用 contract”的混合形态。旧实现可用 `git show fdcd945^:mini_agent/core/agent.py` 复查。

这个形态有三条可以直接复查的失败证据：

1. 模型只要不再请求工具，core 就把一次执行命名为 `completed`（`fdcd945^:mini_agent/core/agent.py:475-491`）；这只能证明模型交回控制权，不能证明用户任务已经完成。
2. 正常回复、取消、模型错误和步数上限都返回字符串，停止原因只存在于可选的同步事件中（`fdcd945^:mini_agent/core/agent.py:400-455,475-501,583-589`；`fdcd945^:mini_agent/core/events.py:14-30`）。
3. CLI 在非交互模式和交互模式都丢弃 `run()` 的返回值，实际显示依赖事件接收器（`fdcd945^:mini_agent/cli.py:584-597,777-798`）。因此字符串既不是主要消费边界，也不能承载持续控制。
...
最终采用“长期 Session + 可寻址 Turn + 内部 Step”，而不是给原字符串再包一层数据类：

```text
CLI / benchmark client
        commands ↓       ↑ events
      AgentSession（长期状态与单个活动 turn 的所有者）
            └─ _AgentLoop（Session 内部实现）
                    └─ step：模型请求 → 工具调用 → 观察结果
...
- 当前 `AgentEventSink` 作为 CLI 的进程内适配器保留；执行相关载荷使用独立快照，但同步回调仍不是未来持久化或多客户端 contract。
...
4. **本轮没有同时实现长任务 supervisor 和基准评测**：先让 Turn 的身份、状态所有权和停止原因可以离线验证；否则上层监督器会建立在含糊的 `completed` 上。现在这些模块有了稳定的执行身份，但仍需各自的消费证据，不能自动视为下一优先级。
