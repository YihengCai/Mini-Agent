...
- 日期：2026-08-25
- 状态：部分推翻（Session 级调用标识符账本见 ADR-0031）
...
2026-08-26 再次复审发现，模型生成的调用标识符只是调用与结果的相关键：隐藏集合不能阻止换 ID 重复同一副作用，也不能跨进程恢复，却永久增长并拒绝已完成批次复用。因此 ADR-0031 删除 Session 级账本；冻结注册、完整批次预检、串行执行、快照、错误归一与成组提交继续保留。
