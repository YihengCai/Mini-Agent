...
本次生产代码净减 106 行，测试净减 110 行并删除 17 项只服务于死代码的测试。完整离线集合保持通过；CLI 文件及其实际使用的 13 个颜色值和宽度算法没有改变，因此没有把 CLI 降为演示宿主。

2026-08-26：后续审计又删除 4 项只重组 ASCII、emoji 与中文既有路径的“真实场景”测试；宽度算法的 9 项分支回归和 1 项导出回归保留。相对改动前累计测试净减 146 行、删除 21 项，定向集合实测 `10 passed in 0.29s`，完整离线集合实测 `253 passed, 5 deselected in 12.85s`。
