...
- 日期：2026-08-24
- 状态：已采纳
- 关联：`mini_agent/core/`、`mini_agent/cli_events.py`、`tests/test_agent_loop_offline.py`、`tests/test_architecture_boundaries.py`、提交 `fe6a682`、`cd9ae14`、`d0f59c5`
...
- `tests/test_architecture_boundaries.py:14-52` 静态拒绝 core 导入 UI、日志或传输模块，并拒绝发行配置重新出现 ACP。
...
- README 所列离线命令在 2026-08-24 实测为 `124 passed`，只有一条既有的 `cache_dir` 配置警告。
