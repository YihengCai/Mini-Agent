...
主 agent loop 与摘要都调用同一个 `generate()`；前者传工具列表，后者省略 `tools`，见 `mini_agent/core/agent.py:329-340,424-434`。两处还都会捕获模型异常，见 `mini_agent/core/agent.py:353-372,435-455`。因此测试替身既要给两类调用提供不同响应，也要在异常被捕获后保留违规证据。
...
- **修改生产 contract**：现有 `Agent` 已能通过鸭子类型注入替身，见 `mini_agent/core/agent.py:36-48`，无需扩大改动。如果运行时需要统一的调用追踪、计费或模型路由，显式用途参数反而可能成为正确的生产设计，但应由对应需求推动。
...
后续 [ADR-0006](0006-remove-legacy-local-compaction.md) 删除了唯一的摘要模型调用；本 ADR 自己列出的“只有一种模型调用时，无标签 FIFO 反而更简单”条件因此成立。用途标签与用途错位测试被删除，全局 FIFO、请求快照、耗尽、未消费结果和首个违规保持可见仍保留。
