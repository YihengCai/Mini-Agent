...
2026-08-26：[ADR-0028](0028-config-file-matches-runtime-model.md) 删除逐字段迁移分支后，旧 `workspace_dir` 改由严格模型作为普通未知字段拒绝，不再附带 `--workspace` 专用提示。CLI 单一持有工作区的能力和选择顺序不变，本决策仍然有效。

2026-08-26：[ADR-0034](0034-cli-composes-workspace-fact.md) 完成这里的“CLI 单一持有”：CLI 现在也组装模型可见工作区事实，`AgentSession` 不再接收或保存路径。原决定中“不改变 Session 工作区事实”只描述 ADR-0024 当轮边界；选择顺序、目录和工具行为仍未改变。
