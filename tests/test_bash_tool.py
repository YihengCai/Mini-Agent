...
async def background_tools():
    ...
    tools = (
        ...
        BashOutputTool(manager=manager),
        BashKillTool(manager=manager),
    )
    try:
        yield tools
    finally:
...
async def test_foreground_command():
    ...
    bash_tool = BashTool(manager=BackgroundShellManager())
...
@pytest.mark.asyncio
async def test_background_command(background_tools):
    """Test running a command in the background."""
    print("\n=== Testing Background Command ===")

    _, bash_tool, bash_output_tool, bash_kill_tool = background_tools
...
@pytest.mark.asyncio
async def test_bash_output_monitoring(background_tools):
...
@pytest.mark.asyncio
async def test_bash_output_with_filter(background_tools):
...
@pytest.mark.asyncio
async def test_bash_kill(background_tools):
    """Test terminating a background command."""
    print("\n=== Testing Bash Kill ===")

    manager, bash_tool, _, bash_kill_tool = background_tools
...
async def test_bash_kill_nonexistent():
    ...
    bash_kill_tool = BashKillTool(manager=BackgroundShellManager())
...
async def test_bash_output_nonexistent():
    ...
    bash_output_tool = BashOutputTool(manager=BackgroundShellManager())
...
@pytest.mark.asyncio
async def test_multiple_background_commands(background_tools):
